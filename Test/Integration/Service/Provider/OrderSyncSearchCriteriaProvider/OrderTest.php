...
class OrderTest extends TestCase
{
    ...
    private const PROVIDER_VIRTUAL_TYPE = 'Klevu\AnalyticsOrderSync\Service\Provider\OrderSyncSearchCriteriaProvider\Order'; // phpcs:ignore Generic.Files.LineLength.TooLong
    ...
    public function testGetSearchCriteria_NoArguments_WithExcludedStatuses(): void
    {
        ...
        $existingStoreIds = array_map(
            static fn (StoreInterface $existingStore): int => (int)$existingStore->getId(),
            $existingStores,
        );
        $existingStoreFilters = array_filter(
            $filters,
            static fn (Filter $filter): bool => (
                'store_id' === $filter->getField()
                && 'eq' === $filter->getConditionType()
                && in_array(
                    needle: (int)$filter->getValue(),
                    haystack: $existingStoreIds,
                    strict: true,
                )
            ),
        );
        $this->assertCount(count($existingStoreIds), $existingStoreFilters);

        $testStoreFilters = array_filter(
            $filters,
            static fn (Filter $filter): bool => (
                'store_id' !== $filter->getField()
                || 'eq' !== $filter->getConditionType()
        ...
        $testStoreFilter = array_shift($testStoreFilters);
    ...
    public function testGetSearchCriteria_SyncStatuses(): void
    {
        ...
        $filter = current($filters);
        $this->assertSame('sync_status', $filter->getField());
        $this->assertSame('in', $filter->getConditionType());
    ...
    public function testGetSearchCriteria_SyncStatuses_WithNotRegistered(): void
    {
        ...
        $this->assertSame('sync_status', $notRegisteredFilter->getField());
        ...
        $this->assertSame('sync_status', $registeredFilter->getField());
    ...
    public function testGetSearchCriteria_CombinationFilters(): void
    {
        ...
        $this->assertSame('sync_status', $statusesFilter->getField());
     ...
     * @return string[]
     */
    private function getExpectedFqcns(): array // @phpstan-ignore-line Used by traits
    {
