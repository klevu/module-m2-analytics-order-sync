...
class MarkOrderAsProcessing implements MarkOrderAsProcessingActionInterface
{
    /**
    ...
    public function execute(
        ...
        if (Statuses::PROCESSING === $currentStatus) {
            $success = false;
            $syncOrderHistoryResult = Results::NOOP;
            $messages[] = 'Order is already processing';
        } else {
            $syncOrderRecord->setStatus(Statuses::PROCESSING->value);
            $syncOrderRecord->setAttempts(
                $syncOrderRecord->getAttempts() + 1,
            );
            try {
